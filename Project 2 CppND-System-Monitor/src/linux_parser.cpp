...
long LinuxParser::ActiveJiffies(int pid) { 
  ...
  if (stream.is_open()) {
    ...
    //convert the parsed value into seconds and verify they are not null
    if (std::all_of(vals[13].begin(),vals[13].end(), isdigit))
      utime = stol(vals[13]);
    if (std::all_of(vals[14].begin(),vals[14].end(), isdigit))
      stime = stol(vals[14]);
    if (std::all_of(vals[15].begin(),vals[15].end(), isdigit))
      cutime = stol(vals[15]);
    if (std::all_of(vals[16].begin(),vals[16].end(), isdigit))
      cstime = stol(vals[16]);  
  }
...
string LinuxParser::User(int pid) { 
  ...
  if (stream.is_open()) {
    while (std::getline(stream, line)) {
      //use replace first or user name shows NA
      std::replace(line.begin(), line.end(), ':', ' ');
      std::istringstream linestream(line);
