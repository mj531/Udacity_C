...
ChatLogic::~ChatLogic()
{
    ...
    ////
    /*
    //TASK5 no longer need delete since it is smart pointer
    // delete chatbot instance
    delete _chatBot;

    //TASK /4 not needed, will cleanup by smart pointer delete all nodes 
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        //TASK3 can't pass a unique ptr by its value with lamda, so pass the node by reference (&node)
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            //TASK3 can't pass a unique ptr by its value with lamda, so pass the node by reference (&node) std::unique_ptr<GraphNode> &node
                            ...
                            //TASK3/4 replace with smart pointer
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);
                            edge->SetChildNode((*childNode).get()); // use get to retun the stored raw pointers
                            ...
                            //TASK3 replace with smart pointer
                            (*childNode)->AddEdgeToParentNode(edge.get());
                            // (*parentNode)->AddEdgeToChildNode(edge.get()); 
                            //TASK4 can't pass unique ptr to a mentod by its valus which creates a copy (forbidden)
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            {
               //TASK3
               rootNode = (*it).get(); // assign current node to root OR it -> get()
            }
            else
            {
    ...
    //TASK5 create stack chatBot, pointer and set handle to chatlogic   
    //allocate the instance in the stack memory
    ChatBot chatBot("../images/chatbot.png");
    // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
    chatBot.SetChatLogicHandle(this);
    // _chatBot = &chatBot;
    // _chatBot->SetRootNode(rootNode);
    // //TASK3 dereference ptr to get object
